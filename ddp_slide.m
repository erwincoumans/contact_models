function [x,u,cst,iter] = ddp_slide(params, op, x0, u0)
...
if (exist('op','var') && isfield(op,'plot') && (op.plot > 0))
    ...
    ph = plot(time,x1(1,:),'b',time,x1(4,:)-x1(5,:),'g',time,x1(3,:),'r');
...
% final cost
if any(final)
    llf = zeros(1,sum(final));
    llf = llf + 4*(x(2,final) - 0.5*r).^2;
%     llf = llf + 4*(x(3,final) - pi/8).^2;
    lf = double(final);
    lf(final) = llf;
else
    lf = 0;
...
% running cost
lx = 0;
lx = lx + 0.05*sabs(x(4,:)-x(5,:)-2*r, 0.1);
lx = lx + 2.0*sabs(x(1,:) - 0.5*r, 0.1);
% lx = lx + 2.0*sabs(x(1,:) - 0.0*r, 0.1);
% lx = lx + 0.2*sabs(x(3,:) - pi/8, 0.1);
