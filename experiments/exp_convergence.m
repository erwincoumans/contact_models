...
for k = 2:N
    ...
    v_save = v;
    [v, ~] = solver_lcp(v, Fext, M, J, repmat(mu, nc-1, 1), psi, h);
...
for kS = 1:numel(solvers)
    err{kRand,kS} = solvers{kS}(v_save, Fext, M, J, repmat(mu, nc-1, 1), psi, h);
end
end
...
% end
for kS = 1:numel(solvers)
    plot(1:1000, median(em{kS}), styles{kS})%, 'Color', colors(kS,:))
end
