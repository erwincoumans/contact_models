function x_next = forward_convex(params, x, u)
% q = [th; x; y; w; v_x; v_y]
...
Gt = [r_vec(2)*t_hat(1) - r_vec(1)*t_hat(2); t_hat];
...
A = J*G; A = (A + A')/2; % J*(M\J)
c = J*kv;
R = (Rmin + (Rmax - Rmin)*psi/wmax)*eye(size(A));

Alt = [-A(1:nc,:); -U I; -U -I];
blt = [c(1:nc,:) + psi/h; zeros(2*nc,1)];

% Solve optimization
f = quadprog(A + R, c, Alt, blt, [], [], [zeros(nc,1); -inf(nc,1)], inf(2*nc,1), [], ...
